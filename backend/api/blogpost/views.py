...
class BlogpostViewSet(viewsets.ModelViewSet):  # pylint: disable=too-many-ancestors
    ...
    def get_queryset(self):
        ...
        queried_type = self.request.query_params.get('type', None)
        ...
        if queried_topic_title is not None:
            result = result.filter(topic__name=queried_topic_title)
        if queried_type is not None:
            result = result.filter(type=queried_type)
        return result
